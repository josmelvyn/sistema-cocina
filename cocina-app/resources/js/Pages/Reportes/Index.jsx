...
export default function ReportesIndex({ auth, escuelas, facturas, secuencia }) {
    ...
    const disponibles = secuencia
        ? secuencia.numero_final - secuencia.proximo_numero + 1
        : 0;
    const proximoNcf = secuencia
        ? `${secuencia.prefijo}${String(secuencia.proximo_numero).padStart(8, "0")}`
        : "---";

    const imprimirFacturaGlobal = () => {
        if (!data.desde || !data.hasta) {
            alert(
                "⚠️ Por favor, selecciona el rango de fechas arriba (Desde / Hasta)",
            );
            return;
        }
        ...
        if (
            confirm(
                "¿Desea generar una nueva factura oficial para el INABIE? Esto consumirá un NCF.",
            )
        ) {
  ...
  const imprimirRelacionGeneral = () => {
        if (!data.desde || !data.hasta) {
            alert("⚠️ Por favor, selecciona el rango de fechas (Desde / Hasta)");
            return;
        }

        const url = route("reportes.relacionGeneral", {
            desde: data.desde,
            hasta: data.hasta,
        });
        window.open(url, "_blank");
                ...
                <div className="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-8">
                    ...
                    <div className="bg-slate-900 rounded-[2rem] p-8 shadow-2xl border border-slate-800 relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div>
                                <h3 className="text-blue-400 text-[10px] font-bold uppercase tracking-[0.3em] mb-2">
                                    Monitor de Comprobantes Gubernamentales
                                </h3>
                            ...
                            <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700 text-center min-w-[150px]">
                                ...
                                <p
                                    className={`text-4xl font-black ${disponibles < 5 ? "text-red-500 animate-bounce" : "text-green-400"}`}
                                >
                                    {disponibles}
                                </p>
                            </div>
                        ...
                        <div className="w-full bg-slate-800 h-1 mt-6 rounded-full overflow-hidden">
                            <div
                                ...
                                style={{
                                    width: `${(secuencia?.proximo_numero / secuencia?.numero_final) * 100}%`,
                                }}
                            ></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
                        ...
                        <div className="bg-blue-50 p-6 rounded-xl mb-8 border border-blue-100">
                            <h3 className="text-blue-800 font-bold mb-4 uppercase text-[10px] tracking-widest">
                                Paso 1: Definir Periodo
                            </h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="border rounded-xl p-6 bg-white shadow-sm border-gray-200">
                                ...
                                <form
                                    onSubmit={generarRelacionCentro}
                                    className="space-y-4"
                                >
                                    <div>
                                        <select
                            ...
                            <div className="border rounded-xl p-6 bg-slate-50 border-gray-200">
                                ...
                                <p className="text-xs text-gray-600 mb-6 font-semibold">
                                    Genera factura global gubernamental para el
                    ...
                    <div className="border rounded-xl p-6 bg-amber-50 border-amber-200">
                        ...
                        <p className="text-xs text-amber-700 mb-6 font-semibold uppercase tracking-tighter">
                            Listado detallado de todas las entregas por centro y
                            ruta.
                        </p>
                        <button
                            onClick={imprimirRelacionGeneral}
                            className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 rounded-lg shadow-md flex items-center justify-center gap-2 transition"
                        >
                            <span>📋</span> Ver Relación General
                        </button>
                    </div>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {facturas && facturas.length > 0 ? (
                                        ...
                                        <tr>
                                            <td
                                                colSpan="4"
                                                className="p-6 text-center text-gray-400 italic font-bold"
                                            >
